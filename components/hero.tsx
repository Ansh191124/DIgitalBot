...
import { useEffect, useState, useRef } from 'react' // Keep useRef for future use, even if lottieAnimationRef is removed
...
export function Hero() {
    ...
    // Removed the unused lottieAnimationRef for cleanup, as Lottie types were also removed.

    useEffect(() => {
        // Simple delay to enable the initial fade-in transition
    ...
    const toggleCall = () => {
        setIsCallActive(prev => {
            ...
            if (newState) {
                ...
                setTranscript("Waiting for your command...")
                // Simulate assistant response after a short delay
                setTimeout(() => {
                    setIsSpeaking(true)
                    setTranscript("I can help you with scheduling, information lookup, or customer support.")
                }, 1500)
                // Stop speaking simulation after the message
                setTimeout(() => setIsSpeaking(false), 5500)
            } else {
    ...
    return (
        <>
            {/* Using dangerouslySetInnerHTML for custom CSS keyframes/animations */}
            <style dangerouslySetInnerHTML={{
                ...
                @keyframes sound-wave {
                    ...
                    50% { transform: scaleY(2); } /* Adjusted scale for a clearer sound effect */
                }
                .animate-pulse-glow {
            ...
            <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
                {/* Animated Background (Rest of your background JSX) */}
                ...
                <div className={`container mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 transition-all duration-1000 ${isContentVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    ...
                    <div className="relative mx-auto mb-12 w-80 h-80 flex items-center justify-center">
                        ...
                        <div className={`relative w-40 h-40 rounded-full bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600 flex items-center justify-center ${isSpeaking ? 'animate-pulse-glow' : ''} transition-all duration-300`}>
                            ...
                            <div className="relative z-10">
                                <Mic
                                    className={`h-8 w-8 text-white transition-opacity duration-300 ${isSpeaking ? 'opacity-0' : 'opacity-100'}`}
                                />
                                {/* Sound Waves */}
                                {isSpeaking && (
                                        ...
                                        <div className="flex space-x-1">
                                            {[...Array(5)].map((_, i) => (
                                                <div
                                                    ...
                                                    style={{
                                                        height: `${20 + (i % 3) * 5}px`, // Adjusted height for stable look
                                                        animationDelay: `${i * 0.15}s` // Adjusted delay
                                                    }}
                                                ></div>
                                            ))}
                                        </div>
                    ...
                    <div className="mb-8">
                        <div className="inline-block bg-white/10 backdrop-blur-md rounded-2xl px-6 py-4 border border-white/20">
                            ...
                            <p className="text-white/90 text-lg max-w-md mx-auto">{transcript}</p>
                        </div>
                    </div>
