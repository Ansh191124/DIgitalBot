...
export function CallItem({
  ...
  phone: string;
  name?: string;
  result?: string | null | number | object; // allow more types
  duration: string;
  when: string;
  sentiment?: string;
  leadScore?: number | null;
}) {
  // Ensure safeResult is always a string
  const safeResult =
  ...
  const chip =
    safeResult.includes("Qualified")
      ? { text: "Positive", cls: "bg-green-100 text-green-700" }
      : safeResult.includes("Not")
      ? { text: "Negative", cls: "bg-rose-100 text-rose-700" }
    ...
    <div className="card p-5 flex items-center gap-4">
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className={`px-2 py-0.5 rounded-lg ${chip.cls}`}>
            {chip.text}
          </span>
        ...
        <div className="grid grid-cols-4 gap-3 mt-2 text-sm">
          ...
          <div>
            ...
            <div className="font-medium">{safeResult || "-"}</div>
          </div>
      ...
      <div className="text-slate-400">
        {safeResult.includes("Qualified") ? <BadgeCheck /> : <XCircle />}
      </div>
    </div>
  );
}